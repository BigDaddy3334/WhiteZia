...
class WhiteZiaProxyService : Service() {
        ...
        startJob = serviceScope.launch {
            ...
            while (isActive && !stopping) {
                try {
                    ...
                    val failureMessage = "Failed to start WhiteZia proxy: ${error.message ?: error::class.java.simpleName}"
                    ...
                    if (!startedOnce) {
                        logError("Failed to start WhiteZia proxy", error)
                        exitForeground()
                        stopSelf()
                        return@launch
                    }
    ...
    private fun startHttpProxyBridge(resolvedSettings: ResolvedWhiteZiaSettings) {
    ...
    private fun createNotificationChannel() {
        ...
        val channel = NotificationChannel(
            ...
            description = "Shows the active WhiteZia proxy connection"
            setShowBadge(false)
        }
    ...
    private fun buildForegroundNotification(statusText: String): Notification {
        ...
        val stopPendingIntent = PendingIntent.getService(
            this,
            1,
            Intent(this, WhiteZiaProxyService::class.java).setAction(ActionStop),
            pendingIntentFlags,
        )

        return NotificationCompat.Builder(this, NotificationChannelId)
            .setSmallIcon(R.drawable.ic_notification)
            .setContentTitle("WhiteZia Proxy")
    ...
    private fun logInfo(message: String) {
        ...
        WhiteZiaProxyEvents.log(currentSessionId, message)
    ...
    private fun reportFailure(message: String) {
        WhiteZiaProxyEvents.failed(currentSessionId, message)
        sendProxyEvent(BroadcastTypeFailed, message)
    }

    private fun reportReady(message: String) {
        Log.i(Tag, message)
        WhiteZiaProxyEvents.ready(currentSessionId, message)
    ...
    companion object {
        private const val Tag = "WhiteZiaProxyService"
        const val BroadcastAction = "shop.whitezia.client.proxy.EVENT"
        const val BroadcastExtraType = "shop.whitezia.client.proxy.extra.TYPE"
        const val BroadcastExtraSessionId = "shop.whitezia.client.proxy.extra.SESSION_ID"
        const val BroadcastExtraMessage = "shop.whitezia.client.proxy.extra.MESSAGE"
        const val BroadcastTypeLog = "log"
        const val BroadcastTypeReady = "ready"
        const val BroadcastTypeFailed = "failed"
        private const val ActionStart = "shop.whitezia.client.proxy.START"
        private const val ActionStop = "shop.whitezia.client.proxy.STOP"
        private const val ExtraSessionId = "shop.whitezia.client.proxy.extra.SESSION_ID"
        ...
        private const val NotificationChannelId = "whitezia_proxy"
        ...
        fun start(
            ...
            val intent = Intent(context, WhiteZiaProxyService::class.java)
        ...
        fun stop(context: Context) {
            runCatching {
                context.startService(
                    Intent(context, WhiteZiaProxyService::class.java)
                        .setAction(ActionStop),
                )
            }.onFailure { error ->
                Log.w(Tag, "Failed to request proxy service stop", error)
                runCatching {
                    context.stopService(Intent(context, WhiteZiaProxyService::class.java))
                }.onFailure { stopError ->
