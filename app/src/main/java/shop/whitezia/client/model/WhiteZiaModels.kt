...
data class ConnectionProfile(
    ...
    companion object {
        ...
        fun fromSettings(settings: WhiteZiaSettings): ConnectionProfile {
            return ConnectionProfile(
                id = DefaultId,
                name = "Connection",
                serverMode = "custom",
...
data class WhiteZiaSettings(
    ...
    val themeMode: String = WhiteZiaThemeMode.System,
    val languageCode: String = WhiteZiaLanguage.En,
    val resolverText: String = "",
    ...
    val autoTuneEnabled: Boolean = WhiteZiaParallelTest.EnabledByDefault,
    val parallelTestSelectedConfigIds: List<String> = WhiteZiaParallelTest.defaultConfigIds,
    ...
    val splitTunnelMode: String = WhiteZiaOptions.SplitTunnelModeOff,
...
object WhiteZiaScanDefaults {
    const val DefaultWorkerCount = 4
    const val WorkerWarningThreshold = 8
}

object WhiteZiaThemeMode {
    const val System = "system"
    const val Light = "light"
    const val Dark = "dark"
}

object WhiteZiaLanguage {
    const val En = "en"
    const val Fa = "fa"
}

data class WhiteZiaScanState(
    val sessionId: String = "",
    val status: String = WhiteZiaScanStatus.Idle,
    val sourceName: String = "",
    val totalResolvers: Int = 0,
    val completedResolvers: Int = 0,
    val validResolvers: Int = 0,
    val rejectedResolvers: Int = 0,
    val workerCount: Int = WhiteZiaScanDefaults.DefaultWorkerCount,
    ...
    val isRunning: Boolean
        get() = status == WhiteZiaScanStatus.Starting || status == WhiteZiaScanStatus.Running
...
data class WhiteZiaUiState(
    ...
    val scanState: WhiteZiaScanState = WhiteZiaScanState(),
    val scanWorkerCount: String = WhiteZiaScanDefaults.DefaultWorkerCount.toString(),
...
object WhiteZiaOptions {
    const val OperatorMegafonYota = "megafon_yota"
    const val OperatorMts = "mts"
    const val OperatorBeeline = "beeline"
    const val OperatorTele2 = "tele2"
    ...
    val themeModes = listOf(
        Choice(WhiteZiaThemeMode.System, "Auto"),
        Choice(WhiteZiaThemeMode.Light, "Light"),
        Choice(WhiteZiaThemeMode.Dark, "Dark"),
    )
...
fun WhiteZiaSettings.normalizedConnectionProfiles(): List<ConnectionProfile> {
...
fun WhiteZiaSettings.runtimeConnectionSettings(): WhiteZiaSettings {
    ...
    return if (settings.connectionMode == "vpn") {
        settings.copy(
            ...
            listenPort = WhiteZiaRuntimeProxy.ListenPort,
...
fun WhiteZiaSettings.applyAdvancedProfile(profile: AdvancedSettingsProfile): WhiteZiaSettings {
    return copy(
...
fun WhiteZiaSettings.selectAdvancedProfile(profileId: String): WhiteZiaSettings {
...
fun WhiteZiaSettings.saveSelectedAdvancedProfile(): WhiteZiaSettings {
...
fun WhiteZiaSettings.saveCurrentAdvancedProfileAs(name: String): WhiteZiaSettings {
...
fun WhiteZiaSettings.upsertAdvancedProfile(profile: AdvancedSettingsProfile): WhiteZiaSettings {
...
fun WhiteZiaSettings.selectConnectionProfile(profileId: String): WhiteZiaSettings {
    val profiles = normalizedConnectionProfiles()
    val resolverProfiles = normalizedResolverProfiles()
...
fun WhiteZiaSettings.upsertConnectionProfile(profile: ConnectionProfile): WhiteZiaSettings {
...
fun WhiteZiaSettings.upsertResolverProfile(profile: ResolverProfile): WhiteZiaSettings {
...
fun WhiteZiaSettings.exportAllResolverProfilesText(): String {
...
fun WhiteZiaSettings.saveResolverProfileAs(
    name: String,
    resolverText: String,
): WhiteZiaSettings {
...
fun WhiteZiaSettings.moveConnectionProfile(profileId: String, direction: Int): WhiteZiaSettings {
    if (direction == 0) {
        return syncSelectedConnectionProfileFields()
    }
    val profiles = normalizedConnectionProfiles()
...
fun WhiteZiaSettings.moveConnectionProfileToIndex(profileId: String, targetIndex: Int): WhiteZiaSettings {
    val profiles = normalizedConnectionProfiles()
    val customProfiles = profiles.filter { it.serverMode == "custom" }
    val fromIndex = customProfiles.indexOfFirst { it.id == profileId }
    if (fromIndex == -1) {
...
fun WhiteZiaSettings.moveResolverProfile(profileId: String, direction: Int): WhiteZiaSettings {
    if (direction == 0) {
        return syncSelectedConnectionProfileFields()
    }
...
fun WhiteZiaSettings.moveResolverProfileToIndex(profileId: String, targetIndex: Int): WhiteZiaSettings {
    if (profileId == ResolverProfile.DefaultId) {
        return syncSelectedConnectionProfileFields()
    }
    val profiles = normalizedResolverProfiles()
...
fun WhiteZiaSettings.moveAdvancedProfileToIndex(profileId: String, targetIndex: Int): WhiteZiaSettings {
    val customProfiles = normalizedAdvancedProfiles().filter { it.id != AdvancedSettingsProfile.DefaultId }
    val fromIndex = customProfiles.indexOfFirst { it.id == profileId }
    if (fromIndex == -1) {
...
fun WhiteZiaSettings.applyResolverProfileToSelectedConnection(profileId: String): WhiteZiaSettings {
    val resolverProfiles = normalizedResolverProfiles()
...
fun WhiteZiaSettings.clearSelectedResolverProfile(): WhiteZiaSettings {
    val connectionProfiles = normalizedConnectionProfiles()
    val selectedConnection = connectionProfiles.firstOrNull { it.id == selectedConnectionProfileId }
        ?: connectionProfiles.first()
    val updatedConnectionProfiles = connectionProfiles.map { profile ->
...
fun WhiteZiaSettings.updateManualResolverText(resolverText: String): WhiteZiaSettings {
    return clearSelectedResolverProfile()
        .copy(resolverText = resolverText)
        .syncSelectedConnectionProfileFields()
}

fun WhiteZiaSettings.deleteResolverProfile(profileId: String): WhiteZiaSettings {
    if (profileId == ResolverProfile.DefaultId) {
        return syncSelectedConnectionProfileFields()
    }
    val profiles = normalizedResolverProfiles()
...
fun WhiteZiaSettings.deleteConnectionProfile(profileId: String): WhiteZiaSettings {
    val profiles = normalizedConnectionProfiles()
...
fun WhiteZiaSettings.duplicateConnectionProfileCount(): Int {
...
fun WhiteZiaSettings.deleteDuplicateConnectionProfiles(protectedProfileId: String? = null): WhiteZiaSettings {
    val profiles = normalizedConnectionProfiles()
    if (profiles.size <= 1) {
        return syncSelectedConnectionProfileFields()
    }
...
fun WhiteZiaSettings.deleteAdvancedProfile(profileId: String): WhiteZiaSettings {
...
fun WhiteZiaSettings.resetAdvancedSettings(): WhiteZiaSettings {
...
private fun normalizeSplitTunnelMode(raw: String): String {
    return when (raw) {
        WhiteZiaOptions.SplitTunnelModeInclude -> raw
        WhiteZiaOptions.SplitTunnelModeExclude -> raw
        WhiteZiaOptions.SplitTunnelModeOff -> raw
        else -> WhiteZiaOptions.SplitTunnelModeOff
    }
}
...
private fun normalizeThemeMode(raw: String): String {
    return when (raw) {
        WhiteZiaThemeMode.System,
        WhiteZiaThemeMode.Light,
        WhiteZiaThemeMode.Dark,
        -> raw
        else -> WhiteZiaThemeMode.System
    }
}

private fun normalizeLanguageCode(raw: String): String {
    return when (raw) {
        WhiteZiaLanguage.Fa -> WhiteZiaLanguage.Fa
        else -> WhiteZiaLanguage.En
    }
}
...
fun WhiteZiaSettings.resolve(): ResolvedWhiteZiaSettings {
    ...
    return ResolvedWhiteZiaSettings(
