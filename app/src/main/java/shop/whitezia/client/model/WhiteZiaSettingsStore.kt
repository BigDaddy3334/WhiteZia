...
class WhiteZiaSettingsStore(
    ...
    fun load(): WhiteZiaSettings {
        ...
        val parallelTestSelectedConfigIds = WhiteZiaParallelTest.normalizeConfigIds(
        ...
        return WhiteZiaSettings(
            ...
            customResolversEnabled = preferences.getBoolean(
                KeyCustomResolversEnabled,
                defaults.customResolversEnabled,
            ),
            customResolverText = preferences.getString(KeyCustomResolverText, defaults.customResolverText)
                ?: defaults.customResolverText,
            customConnectionSettingsEnabled = preferences.getBoolean(
    ...
    private fun migrateAdvancedDefaultsIfNeeded() {
        ...
        if (currentRevision < StabilityDefaultsRevision) {
            editor
                ...
                .putString(
                    KeyParallelTestSelectedConfigIds,
                    encodePackageNames(WhiteZiaParallelTest.defaultConfigIds),
                )
    ...
    private fun migrateLegacyPreferencesFileIfNeeded() {
        val legacyFile = sharedPreferencesFile(LegacyPreferencesName)
        val currentFile = sharedPreferencesFile(PreferencesName)
    ...
    private fun sharedPreferencesFile(name: String): File {
        return File(File(appContext.applicationInfo.dataDir, "shared_prefs"), "$name.xml")
    }
