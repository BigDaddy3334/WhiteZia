...
import shop.whitezia.client.storm.StormDnsBinaryInstaller
import shop.whitezia.client.storm.StormDnsConfigRenderer

class WhiteZiaScanService : Service() {
    ...
    private fun startScan(intent: Intent?) {
        ...
        scanJob = serviceScope.launch {
            ...
            try {
                ...
                publishState(
                    WhiteZiaScanStateStore.read(applicationContext).copy(
                        sessionId = sessionId,
    ...
    private suspend fun runScan(
        ...
        val validResolvers = WhiteZiaScannerResultStore.normalizeResolverEntries(initialValidResolvers)
            .toCollection(linkedSetOf())
        val rejectedResolvers = WhiteZiaScannerResultStore.normalizeResolverEntries(initialRejectedResolvers)
        ...
        fun aggregateState(status: String, message: String): WhiteZiaScanState {
        ...
        if (validResolvers.isNotEmpty()) {
            WhiteZiaScannerResultStore.appendValidResolvers(applicationContext, validResolvers)
        }
        if (workerInputs.isEmpty()) {
            ...
            WhiteZiaScanRequestStore.delete(applicationContext, sessionId)
            exitForeground()
            stopSelf()
            return@coroutineScope
        }
        publishAggregate(WhiteZiaScanStatus.Running, "Scanning $pendingResolverCount resolvers", true)
        ...
        val jobs = workerInputs.mapIndexed { index, workerInput ->
            async(Dispatchers.IO) {
                try {
                    ...
                    publishAggregate(WhiteZiaScanStatus.Running, "Worker ${workerInput.id} failed", true)
                    false
                }
            }
        }
        ...
        val finalStatus = if (successfulWorkers == workerInputs.size && workerFailures.isEmpty()) {
            ...
            WhiteZiaScanStatus.Failed
        } else {
            WhiteZiaScanStatus.Failed
        }
        val finalMessage = if (finalStatus == WhiteZiaScanStatus.Completed) {
        ...
        if (finalStatus == WhiteZiaScanStatus.Completed) {
            WhiteZiaScanRequestStore.delete(applicationContext, sessionId)
        }
        exitForeground()
        stopSelf()
    ...
    private fun handleWorkerOutput(
        ...
        parseStormDnsConnectionProgressLine(line)?.let { progress ->
            ...
            publishAggregate(WhiteZiaScanStatus.Running, "Scanning", false)
            return
        }

        when (val telemetry = parseStormDnsScanLine(line)) {
            is StormDnsScanTelemetry.Valid -> {
                val resolver = WhiteZiaScannerResultStore.normalizeResolverEntry(telemetry.resolver) ?: return
                var validCount = 0
                var added = false
                synchronized(stateLock) {
                ...
                if (added) {
                    WhiteZiaScannerResultStore.appendValidResolvers(applicationContext, listOf(resolver))
                }
                publishAggregate(WhiteZiaScanStatus.Running, "Found $validCount valid resolvers", false)
            }
            ...
            is StormDnsScanTelemetry.Complete -> {
                ...
                publishAggregate(WhiteZiaScanStatus.Running, "Worker ${workerIndex + 1} completed", false)
            }
            null -> Unit
        }
    }
    ...
    private fun writeScanResults(scanRoot: File, state: WhiteZiaScanState) {
        val resultsDir = File(scanRoot, "results").apply { mkdirs() }
        val scannerResultResolvers = WhiteZiaScannerResultStore.mergeValidResolvers(
            applicationContext,
            state.validResolverEntries,
        )
        File(resultsDir, WhiteZiaScannerResultStore.ResultFileName).writeText(
    ...
    private fun stopScanRuntime(message: String) {
        ...
        if (currentSessionId.isNotBlank()) {
            val previous = WhiteZiaScanStateStore.read(applicationContext)
            if (
            ...
            publishState(
                WhiteZiaScanStateStore.stopped(
    ...
    private fun publishState(state: WhiteZiaScanState) {
        if (stopRequested && state.status != WhiteZiaScanStatus.Stopped) {
            return
        }
        WhiteZiaScanStateStore.write(applicationContext, state)
        WhiteZiaScanEvents.state(state)
    ...
    private fun notificationTextFor(state: WhiteZiaScanState): String {
        return when (state.status) {
            WhiteZiaScanStatus.Running,
            WhiteZiaScanStatus.Starting -> "Scanning ${state.completedResolvers}/${state.totalResolvers}"
            WhiteZiaScanStatus.Completed -> "Scan completed: ${state.validResolvers} valid"
            WhiteZiaScanStatus.Failed -> "Scan failed"
    ...
    private fun createNotificationChannel() {
        ...
        val channel = NotificationChannel(
            ...
            description = "Shows active WhiteZia resolver scans"
            setShowBadge(false)
        }
    ...
    private fun buildForegroundNotification(statusText: String): Notification {
        ...
        val stopPendingIntent = PendingIntent.getService(
            this,
            1,
            Intent(this, WhiteZiaScanService::class.java).setAction(ActionStop),
            pendingIntentFlags,
        )
        return NotificationCompat.Builder(this, NotificationChannelId)
            .setSmallIcon(R.drawable.ic_notification)
            .setContentTitle("WhiteZia Scan")
    ...
    companion object {
        private const val Tag = "WhiteZiaScanService"
        const val BroadcastAction = "shop.whitezia.client.scan.EVENT"
        const val BroadcastExtraType = "shop.whitezia.client.scan.extra.TYPE"
        const val BroadcastExtraSessionId = "shop.whitezia.client.scan.extra.SESSION_ID"
        const val BroadcastExtraMessage = "shop.whitezia.client.scan.extra.MESSAGE"
        const val BroadcastTypeState = "state"
        private const val ActionStart = "shop.whitezia.client.scan.START"
        private const val ActionStop = "shop.whitezia.client.scan.STOP"
        private const val ExtraSessionId = "shop.whitezia.client.scan.extra.SESSION_ID"
        private const val OutputDrainJoinMillis = 500L
        private const val NotificationId = 3301
        private const val NotificationChannelId = "whitezia_scan"
        ...
        fun startPrepared(
            ...
            val intent = Intent(context, WhiteZiaScanService::class.java)
        ...
        fun stop(context: Context) {
            runCatching {
                context.startService(
                    Intent(context, WhiteZiaScanService::class.java)
                        .setAction(ActionStop),
                )
            }.onFailure { error ->
                Log.w(Tag, "Failed to request scan service stop", error)
                runCatching {
                    context.stopService(Intent(context, WhiteZiaScanService::class.java))
                }
            }
        }
    }
